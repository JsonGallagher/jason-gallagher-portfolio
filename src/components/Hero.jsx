  ...
  return (
    <section className="min-h-screen flex flex-col justify-center items-center text-center px-6 pt-32 pb-16">
      ...
      <motion.p
        ...
        className="text-lg md:text-xl text-text-secondary dark:text-text-light/80 max-w-2xl mb-10 leading-relaxed text-pretty"
      >
        Demand gen, analytics, automation, and AI strategy.
        <br />
      ...
      >
        {stats.map((stat, i) => (
          <div key={i} className="text-center flex flex-col items-center">
            <div
              className={`font-serif text-3xl md:text-4xl tracking-normal relative flex items-center justify-center gap-1 ${stat.arrow === "up" ? "translate-x-1" : ""} ${stat.nudge || ""}`}
            >
              {stat.arrow === "down" && (
                <ArrowDown className="w-5 h-5 md:w-6 md:h-6 text-green-500" />
              )}
              {stat.arrow === "up" && (
                <ArrowUp className="w-5 h-5 md:w-6 md:h-6 text-green-500 absolute -left-6 md:-left-7 top-1/2 -translate-y-1/2" />
              )}
              {stat.value}
            </div>
      ...
      >
        ...
        <div className="relative flex items-center gap-2 px-4 py-3 bg-gray-50 dark:bg-white/5 border-b border-black/5 dark:border-white/10 group/header">
          <div className="w-3 h-3 rounded-full bg-red-400 flex items-center justify-center">
            <X
              className="w-2 h-2 text-red-900 opacity-0 group-hover/header:opacity-100 transition-opacity"
              strokeWidth={3}
            />
          </div>
          <div className="w-3 h-3 rounded-full bg-yellow-400 flex items-center justify-center">
            <Minus
              className="w-2 h-2 text-yellow-900 opacity-0 group-hover/header:opacity-100 transition-opacity"
              strokeWidth={3}
            />
          </div>
          <div className="w-3 h-3 rounded-full bg-green-400 flex items-center justify-center">
            <Maximize2
              className="w-2 h-2 text-green-900 opacity-0 group-hover/header:opacity-100 transition-opacity rotate-90"
              strokeWidth={3}
            />
          </div>
